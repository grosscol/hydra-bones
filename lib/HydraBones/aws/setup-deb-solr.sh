...
then
  ...
  sudo wget $SOLR_TAR_LOC
fi
# Check that Solr was obtained from the location
if [ -e /usr/local/solr/${SOLR_VER}.tgz ]
then
  sudo tar -xzf ${SOLR_VER}.tgz --keep-newer-files
else
...
then
  ...
  sudo cp /usr/local/solr/${SOLR_VER}/dist/${SOLR_VER}.war  /var/lib/tomcat8/webapps/solr.war
fi
...
then
  ...
  echo "JAVA_OPTS=\"\${JAVA_OPTS} -Dsolr.solr.home=${SOLR_HOME}\"" | sudo tee --append /etc/default/tomcat8
fi
...
then
  ...
  echo "\n# Add the solr.data.dir option.  Without this, Solr will not start." | sudo tee --append /etc/default/tomcat8
  echo "JAVA_OPTS=\"\${JAVA_OPTS} -Dsolr.data.dir=${SOLR_DATA}\"" | sudo tee --append /etc/default/tomcat8
fi
...
sudo chown -R tomcat8:tomcat8 ${SOLR_HOME}

# Copy schemaless files from solr package over to SOLR_HOME
sudo cp -r /usr/local/solr/${SOLR_VER}/example/example-schemaless/* /opt/solr/schemaless

# Copy required library jars to tomcat
sudo cp /usr/local/solr/${SOLR_VER}/example/lib/ext/* /usr/share/tomcat8/lib

# Remove the log4j libraries,  because they prevent Fedora4 from working
sudo rm /usr/share/tomcat8/lib/log4j*.jar
sudo rm /sur/share/tomcat8/lib/slf4j-log4j*.jar
